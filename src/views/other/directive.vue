  ...
  <ComWrapper>
    <el-card shadow="never" header="v-auth 高精度权限控制">
		...
		<el-card shadow="never" header="v-role 角色权限控制" style="margin-top: 15px;">
			...
			<el-alert title="v-role指令 是$ROLE的语法糖, 原理是判断是否含有用户所在的角色别名" style="margin-top: 20px;" />
		</el-card>
		<el-card shadow="never" header="v-time 时间转换" style="margin-top: 15px;">
			<p>
				<el-tag v-time="1630117968295" format="yyyy-MM-dd hh:mm:ss" />
			</p>
			<p style="margin-top: 15px;">
				<el-tag v-time.tip="time1" />
			</p>
			<p style="margin-top: 15px;">
				<el-tag v-time.tip="time2" />
			</p>
			<p style="margin-top: 15px;">
				<el-tag v-time.tip="time3" />
			</p>
			<el-alert title="指令方式日期时间转换,如设置'tip'修饰符将会转换成相对时间,并且每60秒自动更新" style="margin-top: 20px;" />
		</el-card>
		<el-card shadow="never" header="v-copy 一键复制" style="margin-top: 15px;">
			...
			<el-alert title="点击复制按钮会将文本框绑定的值复制到剪切板, 试着粘贴到其他地方看看效果" style="margin-top: 20px;" />
		</el-card>
  </ComWrapper>
</template>
...
const time2 = new Date().setMinutes(new Date().getMinutes() - 1)
const time3 = new Date().setMinutes(new Date().getMinutes() - 120)
const copyText = '测试复制内容'
</script>