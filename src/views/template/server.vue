		...
		<el-aside width="220px">
			<el-container>
				...
				<el-main class="nopadding">
					<el-tree ref="groupRef" class="menu" :data="group" node-key="code" :current-node-key="group[0].code" highlight-current :filter-node-method="groupFilterNode" />
				</el-main>
			</el-container>
		</el-aside>
								...
								<el-tab-pane label="CPU" lazy>
									<div class="server-item-info">
										...
										<div class="chart" style="border-color: #409EFF;background: rgba(64,158,255,0.05);">
											<Echarts ref="cpuRef" height="100%" :option="option.cpu" />
										</div>
										<div class="data">
								...
								<el-tab-pane label="Memory" lazy>
									<div class="server-item-info">
										...
										<div class="chart" style="border-color: #8b12ae;background: rgba(139,18,174,0.05);">
											<Echarts ref="memoryRef" height="100%" :option="option.memory" />
										</div>
									</div>
								</el-tab-pane>
								...
								<el-tab-pane label="Network" lazy>
									<div class="server-item-info">
										...
										<div class="chart" style="border-color: #a74f01;background: rgba(167,79,1,0.05);">
											<Echarts ref="networkRef" height="100%" :option="option.network" />
										</div>
									</div>
								</el-tab-pane>
							</el-tabs>
				...
				<el-tab-pane label="报警管理" lazy>
					<el-main>
						<el-card shadow="never" header="Event">
							<Table ref="tableRef" :data="event" stripe height="auto">
								<el-table-column label="#" type="index" width="50" />
								<el-table-column label="事件" prop="title" width="150" />
								<el-table-column label="等级" prop="type" width="150" />
								<el-table-column label="通知" prop="push" />
								<el-table-column label="触发时间" prop="time" width="180" />
							</Table>
						</el-card>
						<el-card shadow="never" header="Rule" style="margin-top: 15px;">
							<el-table :data="rule" stripe>
								...
								<el-table-column label="描述" prop="title" width="150" />
								<el-table-column label="告警类型" prop="type" width="150" />
								<el-table-column label="通知" prop="push" width="150" />
								<el-table-column />
...
import {groups, colorlist, events, rules, options} from '@/plugins/mock'
...
const rule = ref(rules)
const option = ref(options)

/* onMounted */
onMounted(() => {
  setInterval(() => {
    option.value.cpu.series[0].data.shift()
    option.value.cpu.series[0].data.push(Math.round(Math.random() * 100))
    option.value.cpu.xAxis.data.shift()
    option.value.cpu.xAxis.data.push((new Date()).toLocaleTimeString().replace(/^\D*/, ''))

    option.value.memory.series[0].data.shift()
    option.value.memory.series[0].data.push(Math.round(Math.random() * 50))
    option.value.memory.xAxis.data.shift()
    option.value.memory.xAxis.data.push((new Date()).toLocaleTimeString().replace(/^\D*/, ''))

    option.value.network.series[0].data.shift()
    option.value.network.series[0].data.push(Math.round(Math.random() * 20))
    option.value.network.series[1].data.shift()
    option.value.network.series[1].data.push(Math.round(Math.random() * 10))
    option.value.network.xAxis.data.shift()
    option.value.network.xAxis.data.push((new Date()).toLocaleTimeString().replace(/^\D*/, ''))
  }, 1000)
})

/* watch */
watch(groupFilterText, newVal => groupRef.value.filter(newVal))
...
async function tabClick(tab) {
  ...
  const cpuRefVal = cpuRef.value
  const memoryRefVal = memoryRef.value
  const networkRefVal = networkRef.value
  if (label === 'CPU' && cpuRefVal.myChart) {
    cpuRefVal.myChart.resize()
  }
  if (label === 'Memory' && memoryRefVal.myChart) {
    memoryRefVal.myChart.resize()
  }
  if (label === 'Network' && networkRefVal.myChart) {
    networkRefVal.myChart.resize()
  }
}
</script>
