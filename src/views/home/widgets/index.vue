  ...
  <div :class="['widgets-home', customizing ? 'customizing' : '']" ref="main">
			...
			<div class="widgets" ref="widgets">
				<div class="widgets-wrapper">
					<div v-if="nowCompsList.length <= 0" class="no-widgets">
						<el-empty image="../../../assets/img/no-widgets.svg" description="没有部件啦" :image-size="280" />
					</div>
					<el-row :gutter="15">
		...
		<div v-if="customizing" class="widgets-aside">
			<el-container>
				...
				<el-header style="height:auto; padding: 13px 15px">
				...
				<el-footer style="height:51px; padding: 13px 15px">
...
import {getItem, setItem, removeItem} from '@/utils/local'
import {GRID} from '@/utils/constant'
...
const allComps = ref(allComp)
const defaultGrid = ref($CONFIG.DEFAULT_GRID)
const grid = ref([])

grid.value = getItem(GRID)|| JSON.parse(JSON.stringify(defaultGrid.value))

onMounted(() => {
  emits('on-mounted', null)
})

const allCompsList = computed(() => {
  let allComponents = []
  for(const key in allComps.value){
    allComponents.push({
      key,
      title: allComp[key].title,
      icon: allComp[key].icon,
      description: allComp[key].description
    })
  }
  let myCopmsList = grid.value.copmsList.reduce((a, b) => a.concat(b))
...
const myCompsList = computed(() => allCompsList.value.filter(item => !item.disabled))
const nowCompsList = computed(() => grid.value.copmsList.reduce((a, b) => a.concat(b)))
console.log(myCompsList);

//开启自定义
async function custom() {
  customizing.value = true
  const widgetsVal = widgets.value
  const oldWidth = widgetsVal.offsetWidth
  await nextTick()
  const scale =  widgetsVal.offsetWidth / oldWidth
  widgetsVal.style.setProperty('transform', `scale(${scale})`)
}
//设置布局
function setLayout(layout) {
  const gridVal = grid.value
  grid.value.layout = layout
  if (layout.join(',') === '24') {
    grid.value.copmsList[0] = [...gridVal.copmsList[0], ...gridVal.copmsList[1], ...gridVal.copmsList[2]]
...
//追加
function push(item){
  let target = grid.value.copmsList[0]
  target.push(item.key)
}
//隐藏组件
function remove(item){
  var newCopmsList = grid.value.copmsList
...
function save(){
  ...
  setItem(GRID, grid.value)
}
//恢复默认
function backDefaul(){
  customizing.value = false
  widgets.value.style.removeProperty('transform')
  grid.value = JSON.parse(JSON.stringify(defaultGrid.value))
  removeItem(GRID)
}
//关闭
function close(){
