  ...
  <div class="sc-file-select">
		<div class="sc-file-select__side" v-loading="menuLoading">
			<div class="sc-file-select__side-menu">
				<el-tree ref="groupRef" class="menu" :data="menu" :node-key="treeProps.key" :props="treeProps" :current-node-key="menu.length ? menu[0][treeProps.key] : ''" highlight-current @node-click="groupClick">
		...
		<div class="sc-file-select__files" v-loading="listLoading">
			<div class="sc-file-select__top">
				...
				<div class="keyword">
					<el-input v-model="keyword" prefix-icon="el-icon-search" placeholder="文件名搜索" clearable @keyup.enter="search" @clear="search" />
				</div>
			</div>
			<div class="sc-file-select__list">
				<el-scrollbar ref="scrollbarRef">
					<el-empty v-if="!fileList.length && !data.length" description="无数据" :image-size="80" />
					<div v-for="(file, index) in fileList" :key="index" class="sc-file-select__item">
						<div class="sc-file-select__item__file">
							<div class="sc-file-select__item__upload">
								<el-progress type="circle" :percentage="file.progress" :width="70" />
							</div>
...
const {
  ...
  pageSize, currentPage, total, data, fileList, value, menu, accept, treeProps, fileProps, files,
...
} = useFileSelect(props, handleEmits)

function handleEmits(val1, val2) {
  val1 && emits('update:modelValue', value)
  val2 && emits('submit', value)
} 
</script>
